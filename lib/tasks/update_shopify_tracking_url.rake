    ...
    Shop.all.each do |shop|
      ...
      child_orders.each do |order|
        ...
        if shopify_order.fulfillments.present?
          if shopify_order.fulfillments.first.tracking_number.present?
            if shopify_order.fulfillments.first.tracking_number.include?("UPS") == true
              if shopify_order.fulfillments.first.tracking_number.split(" ")[1].nil?
                order.shopify_tracking_id = shopify_order.fulfillments.first.tracking_number.split(" ")[1]
              else
                order.shopify_tracking_id = shopify_order.fulfillments.first.tracking_number.split("#")[1]
              end
            else
              order.shopify_tracking_id = shopify_order.fulfillments.first.tracking_number
            end
          end
