class Order < ApplicationRecord
  ...
  def self.to_csv(order, options = {})
    ...
    CSV.generate(headers: true) do |csv|
      ...
      all.each do |product|
        csv << product.attributes.merge(product.try(:billing_address).try(:first_name)).merge(product.try(:shipping_address).try(:first_name)).merge(product.try(:parent_order).try(:order_number)).merge(product.try(:customer).try(:first_name)).values_at(*attributes)
      end
    end
  end
end
