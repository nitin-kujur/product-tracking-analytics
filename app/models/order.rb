class Order < ApplicationRecord
  ...
  def self.to_csv(order, options = {})
    ...
    column_headers = ["order_number", "parent Order", "shop","customer","shopify_order_id","email","closed_at","shopify_created_at","shopify_updated_at","total_price","subtotal_price","financial_status","total_line_items_price","cancelled_at","cancel_reason","fulfillment_status","contact_email","billing_address", "shipping_address_id", "shopify_tracking_id", "amount", "tracking_url", "shipped_date"]
    CSV.generate(write_headers: true, headers: column_headers) do |csv|
      all.each do |s|
        csv << [s.order_number, s.try(:parent_order).try(:order_number), s.shop.shopify_domain, "#{(if s.try(:customer).try(:first_name).nil? ? " " : s.try(:customer).try(:first_name)) + (if s.try(:customer).try(:last_name).nil? ? " " : s.try(:customer).try(:last_name))}", s.shopify_order_id, s.email, s.closed_at, s.shopify_created_at, s.shopify_upadted_at, s.total_price, s.subtotal_price, s.financial_status, s.total_line_items_price, s.cancelled_at, s.cancel_reason, s.fulfillment_status, s.contact_email, "#{(if s.try(:billing_address).try(:first_name).nil? ? " " : s.try(:billing_address).try(:first_name))  + (if s.try(:billing_address).try(:first_name).nil? ? " " : s.try(:billing_address).try(:first_name)}","#{if s.try(:shipping_address).try(:first_name).nil? ? " " : s.try(:shipping_address).try(:first_name)) + if s.try(:shipping_address).try(:last_name).nil? ? " " : s.try(:shipping_address).try(:last_name))}", s.shopify_tracking_id, s.amount, s.tracking_url, s.shipped_date ]
      end
    end
  end
end
