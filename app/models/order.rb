class Order < ApplicationRecord
  ...
  def self.save_shopify_order(shop, shopify_obj)
    ...
    shopify_obj.tags.split(",").each do |order_tag|
      order_t = OrderTag.where(:name => order_tag.split(":")[0].try(:strip), :value => order_tag.split(":")[1].try(:strip)).first
      if order_t.nil?
        order_t = @order.order_tags.build(:name => order_tag.split(":")[0].try(:strip),:value => order_tag.split(":")[1].try(:strip))
      end
    end
    ...
    if shopify_obj.line_items.present?
      shopify_obj.line_items.each do |l|
        ...
        if product.nil?
          ...
          unless shopify_product.nil? 
            ...
            shopify_product.variants.each do |variant|
              ...
              variant = Variant.where(:shopify_variant_id => variant.id).first
              if variant.nil?
                product.variants.build(:shopify_product_id => shopify_product.id, :title => shopify_variant.title, :sku => shopify_variant.sku, :inventory_policy => shopify_variant.inventory_policy, :position => shopify_variant.position, :inventory_quantity => shopify_variant.inventory_quantity, :source => nil, :shopify_variant_id => shopify_variant.id)
              end
            end
            product.save
            @order.order_products.build(:product_id => product.id)
        ...
        @order.line_items.build(:shopify_line_item_id => l.id, :variant_id => l.variant_id, :title => l.title,:quantity => l.quantity, :price => l.price, :sku => l.sku, :fulfillment_service => l.fulfillment_service, :product_id => product.try(:id), :requires_shipping => l.requires_shipping, :properties => l.properties.map(&:attributes), :fulfillable_quantity => l.fulfillable_quantity, :total_discount => l.total_discount, :fulfillment_status => l.fulfillment_status, :destination_location => l.try(:destination_location).try(:attributes), :origin_location => l.try(:origin_location).try(:attributes))
      end
    end  
