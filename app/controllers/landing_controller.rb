class LandingController < ApplicationController
  def index
    ...
    if params[:shipped_to_search].present?
      ...
      puts "================item_search===================="
      if @orders_search.nil?
        @orders_search = @orders.joins(:line_items).where("lower(line_items.title) like ?", "#{params[:item_search].downcase}").where(:cancelled_at => nil).paginate(:page => params[:page], :per_page => 50).where(:cancelled_at => nil)
      else
      ...
      if @orders_search.nil?
        @orders_search = @orders.where("lower(shopify_tracking_id) like ?", "%#{params[:tracking_number].downcase}%").where(:cancelled_at => nil).paginate(:page => params[:page], :per_page => 50).where(:cancelled_at => nil)
      else
      ...
      if @orders_search.nil?
        ...
        @orders_search.marge(@orders.joins(:billing_address).where("lower(addresses.city) || lower(addresses.first_name) || lower(addresses.last_name) || lower(addresses.address1) || lower(addresses.zip) || lower(addresses.name) like ?", "%#{params[:free_text_search].downcase}%").where(:cancelled_at => nil).paginate(:page => params[:page], :per_page => 50).where(:cancelled_at => nil))
    ...
    if params[:sku_search].present?
      puts "================sku_search===================="
      if @orders_search.nil?
        @orders_search = @orders.joins(:line_items).where("lower(line_items.sku) like ?", "%#{params[:sku_search].downcase}%").where(:cancelled_at => nil).paginate(:page => params[:page], :per_page => 50)
      else
