  ...
  def index
    ...
    if params[:form_date].present? && params[:to_date].present? || params[:shop_id].present?
      ...
      if !params[:form_date].empty? && !params[:to_date].empty? && !params[:shop_id].empty?
        ...
        if params[:shop_id] == "all"
          ...
          @cancelled_orders = Order.all.where.not(:cancelled_at => nil)
        else
          @orders = Order.all.where(:shop_id => params[:shop_id]).where(:cancelled_at => nil)
          @cancelled_orders = Order.all.where(:shop_id => params[:shop_id]).where.not(:cancelled_at => nil)
        end  
      elsif !params[:form_date].empty? && !params[:to_date].empty? && params[:shop_id].empty?
        puts "++++++++6. Shop parameter not empty ++++++++++"
        @orders = Order.all.where(:cancelled_at => nil).where("date(shopify_created_at) BETWEEN ? AND ? ", "#{params[:form_date]}","#{params[:to_date]}")
        @cancelled_orders = Order.all.where.not(:cancelled_at => nil).where("date(shopify_created_at) BETWEEN ? AND ? ", "#{params[:form_date]}","#{params[:to_date]}")
      end
    else
    ...
    if params[:shipped_to_search].present?
      ...
      if @orders_search.nil?
        ...
        @order_name2 = @orders.joins(:billing_address).where("lower(addresses.city) || lower(addresses.first_name) || lower(addresses.last_name) || lower(addresses.address1) || lower(addresses.zip) || lower(addresses.name) like ?", "%#{params[:free_text_search].strip.downcase}%").where(:cancelled_at => nil)
        @order_name3 = @orders.joins(:shipping_address).where("lower(addresses.city) || lower(addresses.first_name) || lower(addresses.last_name) || lower(addresses.address1) || lower(addresses.zip) || lower(addresses.name) like ?", "%#{params[:free_text_search].strip.downcase}%").where(:cancelled_at => nil)
