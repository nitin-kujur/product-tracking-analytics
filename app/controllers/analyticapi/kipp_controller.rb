class Analyticapi::KippController < ApplicationController
  ...
  def search_orders
	...
	if params[:domain].present? && params[:search_term].present?
	 		...
	 		@orders = shop.orders.joins(:customer).where("lower(order_number) like ?", "%#{params[:search_term].strip.downcase}%" ).where("lower(customers.first_name) || lower(customers.last_name) ?", "%#{params[:search_term].strip.downcase}%").where(:cancelled_at => nil)
